...
class LIWCFeature(BaseEstimator,TransformerMixin):
    ...
    def transform(self, documents):
        ...
        for doc in documents:
            ...
            #print("jhsvdjhAVDHJavdhVDHKagvsd")
            #f = round(you_emo/len(tokens), 2)
            #print(f)
            #print([death_emo, sexual_emo, anger_emo, sad_emo, you_emo, swear_emo, len(tokens)])
            #print([death, sexual, anger, sad, you, swear])
            #print([round(death_emo/len(tokens), 2), round(sexual_emo/len(tokens), 2), round(anger_emo/(len(tokens)),2),
            #       round(sad_emo/len(tokens), 2), round(you_emo/len(tokens), 2), round(swear_emo/(len(tokens)),2)])
            #fea_emo.append(total)


        #print(fea_you_emo)
        #X = np.array([fea_death_emo, fea_sexual_emo, fea_anger_emo, fea_sad_emo]).T
